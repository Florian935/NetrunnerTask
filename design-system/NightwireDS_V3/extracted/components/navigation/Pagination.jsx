...
export function Pagination({ page = 1, pageCount = 1, onChange, style = {}, ...rest }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', ...style }} {...rest}>
      {cell(<Icon name="chevron-left" size={15} />, { disabled: page <= 1, onClick: () => go(page - 1) })}
      {pages.map((p, i) => p === '…'
        ? <span key={`e${i}`} style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', padding: '0 2px' }}>…</span>
        : <React.Fragment key={p}>{cell(p, { active: p === page, onClick: () => go(p) })}</React.Fragment>)}
      {cell(<Icon name="chevron-right" size={15} />, { disabled: page >= pageCount, onClick: () => go(page + 1) })}
    </div>
  );
}
